        ...
        while not all_correct:
            ...
            for x, y in dataset.iterate_once(batch_size):
                ...
                if pred != nn.as_scalar(y):
                    multiplier = nn.Constant(nn.as_scalar(y) * x.data)
                    self.w.update(multiplier, learning_rate)
                    all_correct = False
    ...
    def train(self, dataset):
        ...
        while loss >= loss_threshold:
            for x, y in dataset.iterate_once(batch_size):
                ...
                gradients = nn.gradients(loss_node, parameters)
                for i in range(len(parameters)):
                    parameters[i].update(gradients[i], -learning_rate)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        h = nn.ReLU(z)
        for elem in xs[1:]:
            m1 = nn.Linear(h, self.w2)
            m2 = nn.Linear(elem, self.wx)
            z = nn.AddBias(nn.Add(m1, m2), self.b2) 
            h = nn.ReLU(z)
        pred = nn.AddBias(nn.Linear(h, self.w3), self.b3)
        return pred
