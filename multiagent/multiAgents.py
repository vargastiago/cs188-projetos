...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        foodDistances = [manhattanDistance(pos, newPos)
                         for pos in newFood.asList()]
        minFoodDistances = min(foodDistances)
        newScore = successorGameState.getScore()
        
        if (successorGameState.getNumFood() < currentGameState.getNumFood()):
            return newScore - minFoodDistances + 10
        return newScore - minFoodDistances
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for a in gameState.getLegalActions(0):
            ...
            value = self.getValue(successor, 0, 1)
    ...
    def maxValue(self, gameState, depth):
        ...
        for a in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, a)
            v = max(v, self.getValue(successor, depth, 1))
        return v
    ...
    def minValue(self, gameState, depth, agentIndex):
        ...
        for a in gameState.getLegalActions(agentIndex):
            ...
            if agentIndex == gameState.getNumAgents() - 1:
                v = min(v, self.getValue(successor, depth + 1, 0))
            else:
                v = min(v, self.getValue(successor, depth, agentIndex + 1))
        return v
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for a in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, a)
            value = self.getValue(successor, 0, 1, alpha, beta)
            if value > maximumValue:
                maximumValue = value
                action = a
    ...
    def maxValue(self, gameState, depth, alpha, beta):
        ...
        for a in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, a)
            v = max(v, self.getValue(successor, depth, 1, alpha, beta))
            if v > beta:
                return v
            alpha = max(alpha, v)
        return v
    ...
    def minValue(self, gameState, depth, agentIndex, alpha, beta):
        ...
        for a in gameState.getLegalActions(agentIndex):
            successor = gameState.generateSuccessor(agentIndex, a)
            if agentIndex == gameState.getNumAgents() - 1:
                v = min(v, self.getValue(successor, depth + 1, 0, alpha, beta))
            else:
                v = min(v, self.getValue(successor, depth,agentIndex + 1, alpha, beta))
            if v < alpha:
                return v
            beta = min(beta, v)
        return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def expectValue(self, gameState, depth, agentIndex):
        ...
        for a in gameState.getLegalActions(agentIndex):
            successor = gameState.generateSuccessor(agentIndex, a)
            if agentIndex == gameState.getNumAgents() - 1:
                v += self.getValue(successor, depth + 1, 0)
            else:
                v += self.getValue(successor, depth, agentIndex + 1)
        return v
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    ghostDistances = [manhattanDistance(currentGameState.getGhostPosition(i), newPos)
                      for i in range(1, currentGameState.getNumAgents())]
    if min(ghostDistances) < 2:
        return float('-inf')
    newFood = currentGameState.getFood()
    foodDistances = [manhattanDistance(pos, newPos) for pos in newFood.asList()]
    return (scoreEvaluationFunction(currentGameState) - 
            2 * min(foodDistances) - max(foodDistances) -
            7 * currentGameState.getNumFood() +
            1.5 * min(ghostDistances) + max(ghostDistances))
